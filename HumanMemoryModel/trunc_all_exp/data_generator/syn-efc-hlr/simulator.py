...
class StudentEnv:
    ...
    def step(self, action):
        ...
        history_right = math.sqrt(1+self.success_count_array[action])
        history_wrong = math.sqrt(1+self.fail_count_array[action])
        ...
        tlasts, nreps = self.encode_obs(action, curr_outcome)
        info = [self.prev_memory, curr_outcome, [tlasts, history_right, history_wrong], nreps]
        return done, info
    ...
    def encode_obs(self, item, outcome):
        ...
        tlasts = (self.now - self.last_recall_time_array[item]) / self.const_delay
        ...
        return tlasts, self.count_array[item]
